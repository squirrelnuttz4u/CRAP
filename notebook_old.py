# notebook.py
# © 2025 Colt McVey
# The core notebook widget, with a reactive execution model.
...
# Import collaboration client and other services
...
def ansi_to_html(ansi_string: str) -> str:
    color_map = {
...
class BaseCell(QFrame):
    content_changed = Signal(object) # Emits self
...
class CodeCell(TextEditorCell):
    ...
    def __init__(self, kernel, engine, content=""):
        super().__init__()
        self.kernel = kernel; self.engine = engine
...
class NotebookWidget(QWidget):
    ...
    def add_cell(self, cell_type: str, content="", cell_id=None, from_remote=False, at_index=-1):
        if cell_type == 'code':
            cell = CodeCell(self.kernel, self.engine, content)
    ...
    def on_refactor_requested(self, cell: CodeCell):
        ...
        cell.output_area.setText("<i>Vibe Check in progress...</i>")
        cell.output_area.show()

        asyncio.create_task(self.run_refactor(cell, code_to_refactor))

    async def run_refactor(self, cell: CodeCell, code: str):
        """Runs the refactoring task asynchronously."""
        ...
        streams = await self.engine.battle([chat_model], messages)
        refactored_code = "".join([token async for token in streams[0]])
    ...
    def on_generate_action_requested(self, cell: CodeCell, action_type: str):
        ...
        cell.output_area.setText(f"<i>Generating {action_type}...</i>")
        cell.output_area.show()

        asyncio.create_task(self.run_generation_task(cell, code_to_process, action_type))

    async def run_generation_task(self, cell: CodeCell, code: str, action_type: str):
        """Runs the generation task asynchronously."""
        ...
        streams = await self.engine.battle([chat_model], messages)
        generated_content = "".join([token async for token in streams[0]])
    ...
    def run_all_tests(self):
        ...
        if not self.execution_queue:
            reply = QMessageBox.question(self, "No Tests Found", 
                                         "No test cells were found.\n\nWould you like to run all code cells instead?",
    ...
    def on_cell_content_changed(self):
        sender_cell = self.sender()
        if isinstance(sender_cell, BaseCell):
            self.set_dirty(True)
            message = {"type": "cell_update", "cell_id": sender_cell.cell_id, "content": sender_cell.get_content()}
            self.collab_client.send_message(message)
